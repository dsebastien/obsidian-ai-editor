...
| M2 buffered review           | **PARTIAL (code, unwired)**             | Landed as tested units: `RunController`/`ReviewRunHandle` + `FindingStore` (streaming ingestion, cancellation, late-finding remap through user edits, error redaction seam) in `src/app/services/orchestration/`; context assembly + exclusions (fail-closed, typed `ExcludedTargetError`) in `src/app/services/context/`; CM6 skeleton (finding decorations, rail model/view, `ChangeDesc`→`TextChange` adapter) in `src/app/ui/editor/`; status-bar item; placeholder `Review current note` command. **OPEN SEAMS for whoever resumes**: (1) `RunController` is NOT wired to the provider builders/SSE transport — no end-to-end review flow yet; (2) CM6 decorations/rail need manual verification in a live vault (Live Preview + Source mode); (3) the `Review current note` command is a Notice-only placeholder and must not reach a release; (4) UI behaviors (salvage Notice, chip rejection) unverified at runtime. |
