...
- **Error messages are status-only — details are behind a click.** The message itself never carries the tool's own error text: an agent CLI echoes its configuration when it fails, and configuration contains credentials. When a run fails, **Show details** (on the failed editor's section, on a failed panel scorecard, on a failed transform's notice, on a failed margin comment's card and panel row, and on a failed Test connection) opens the captured output — exit status, error stream, output tail — with a caveat to check it before sharing, because that text is yours to read, not the plugin's to broadcast. For margin comments the captured output lives only for the current session: after a restart the failed comment keeps its message, but the details are gone.
