...
**Right-click a finding** (or press the context-menu key on it) for its actions without opening the card: **Go to finding**, **Open card**, **Accept** (when the finding carries an applicable proposal), **Dismiss** and **Copy critique**. Accept and Dismiss go through exactly the same path as the card's buttons — Accept re-checks the proposal against the live text and is one undo step. Accept needs the note open in an editor.

**Findings in the note properties.** An editor may comment on the frontmatter (unless **Strip frontmatter** is on, in which case it never sees it). In Live Preview, Obsidian shows the frontmatter as the Properties widget, so those findings have no highlight to click. Their panel row says **In the note properties**, and clicking it opens the finding's card directly; the right-click menu works on them like on any other finding.

**Acknowledging clean editors.** An editor that settled with nothing left to act on — no findings, or everything triaged — shows a small **✓** in its section header. Clicking it acknowledges the editor: its section leaves the list, so the panel shows only what still needs attention. Acknowledged sections are counted in a footer line ("2 all-good editors acknowledged · Show") — never silently omitted — and one comes back on its own the moment its editor reports findings again; a re-review that is still clean stays acknowledged. Per note, and forgotten when Obsidian restarts.
